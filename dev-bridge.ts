import express from 'express';
import { createServer as createViteServer } from 'vite';
import path from 'path';
import { createClient } from '@supabase/supabase-js';
import { v4 as uuidv4 } from 'uuid';
import 'dotenv/config';

// Global error handler for WebSocket rejections
process.on('unhandledRejection', (reason) => {
  if (reason instanceof Error && (reason.message.includes('WebSocket') || reason.message.includes('closed without opened'))) {
    return;
  }
  console.error('Unhandled Rejection:', reason);
});

async function startServer() {
  const app = express();
  const PORT = 3000;

  app.use(express.json());

  // Supabase Admin Client (Bypasses RLS)
  const supabaseAdmin = createClient(
    process.env.SUPABASE_URL || '',
    process.env.SUPABASE_SERVICE_ROLE_KEY || ''
  );

  // Diagnostic Route
  app.get('/api/admin/db-check', async (req, res) => {
    try {
      const { data: t, error: te } = await supabaseAdmin.from('tournaments').select('id', { count: 'exact' });
      const { data: p, error: pe } = await supabaseAdmin.from('players').select('id', { count: 'exact' });
      res.json({ 
        connected: true, 
        tournaments: t, 
        players: p,
        errors: { te, pe }
      });
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
  });

  // Read Routes (Bypass RLS)
  app.get('/api/admin/tournaments', async (req, res) => {
    try {
      const { data, error } = await supabaseAdmin
        .from('tournaments')
        .select('*')
        .order('created_at', { ascending: false });
      if (error) throw error;
      res.json(data);
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
  });

  app.get('/api/admin/tournament/:id', async (req, res) => {
    try {
      const { id } = req.params;
      console.log(`[Bridge] Fetching tournament data for ID: ${id}`);
      const { data: tournament, error: tError } = await supabaseAdmin
        .from('tournaments')
        .select('*')
        .eq('id', id)
        .maybeSingle();

      if (tError) {
        console.error('[Bridge] Tournament fetch error:', tError);
        throw tError;
      }
      if (!tournament) {
        console.warn(`[Bridge] Tournament not found: ${id}`);
        return res.status(404).json({ error: 'Not found' });
      }

      console.log(`[Bridge] Tournament found: ${tournament.name}, fetching related data...`);

      const [
        { data: players, error: pError },
        { data: matches, error: mError },
        { data: playoffTeams, error: ptError },
        { data: playoffMatches, error: pmError }
      ] = await Promise.all([
        supabaseAdmin.from('players').select('*').eq('tournament_id', id).order('rank', { ascending: true }),
        supabaseAdmin.from('matches').select('*').eq('tournament_id', id).order('round', { ascending: true }),
        supabaseAdmin.from('playoff_teams').select('*').eq('tournament_id', id),
        supabaseAdmin.from('playoff_matches').select('*').eq('tournament_id', id)
      ]);

      if (pError) console.error('[Bridge] Players fetch error:', pError);
      if (mError) console.error('[Bridge] Matches fetch error:', mError);
      if (ptError) console.error('[Bridge] Playoff teams fetch error:', ptError);
      if (pmError) console.error('[Bridge] Playoff matches fetch error:', pmError);

      console.log(`[Bridge] Data fetched: ${players?.length || 0} players, ${matches?.length || 0} matches`);

      res.json({
        tournament,
        players: players || [],
        matches: matches || [],
        playoffTeams: playoffTeams || [],
        playoffMatches: playoffMatches || []
      });
    } catch (err: any) {
      console.error('[Bridge] Fatal error in /api/admin/tournament/:id:', err);
      res.status(500).json({ error: err.message });
    }
  });

  // API Routes for Administrative Tasks
  app.post('/api/admin/setup', async (req, res) => {
    try {
      const { name, mode, players } = req.body;
      console.log(`[Admin] Setting up tournament: ${name} (${mode}) with ${players.length} players`);
      
      // 1. Create tournament
      const { data: tData, error: tError } = await supabaseAdmin
        .from('tournaments')
        .insert([{ name, mode, status: 'SETUP', current_round_index: 0 }])
        .select()
        .single();
      
      if (tError) {
        console.error('[Admin] Tournament Insert Error:', tError);
        throw tError;
      }

      console.log(`[Admin] Tournament created with ID: ${tData.id}`);

      // 2. Create players
      const playersToInsert = players.map((p: any) => ({
        id: p.id, // Use the ID from the frontend
        name: p.name,
        phone: p.phone,
        email: p.email,
        dupr_id: p.duprId,
        jersey_number: p.jerseyNumber,
        tournament_id: tData.id,
        rank: 0,
        points: 0,
        point_diff: 0,
        points_scored: 0,
        pod_wins: 0
      }));

      const { error: pError } = await supabaseAdmin
        .from('players')
        .insert(playersToInsert);
      
      if (pError) {
        console.error('[Admin] Players Insert Error:', pError);
        throw pError;
      }

      console.log(`[Admin] ${playersToInsert.length} players inserted successfully`);
      res.json({ id: tData.id });
    } catch (err: any) {
      console.error('[Admin] Setup Route Error:', err);
      res.status(500).json({ error: err.message });
    }
  });

  app.post('/api/admin/start-seeding', async (req, res) => {
    try {
      const { tournamentId } = req.body;
      
      const { data: players, error: pError } = await supabaseAdmin
        .from('players')
        .select('*')
        .eq('tournament_id', tournamentId);
      if (pError) throw pError;

      const shuffled = [...players].sort(() => Math.random() - 0.5);
      const matches: any[] = [];
      
      for (let i = 0; i < shuffled.length; i += 4) {
        const podPlayers = shuffled.slice(i, i + 4);
        if (podPlayers.length < 4) break;

        const podId = uuidv4();
        const podName = String.fromCharCode(65 + (i / 4));
        const matchTemplates = [[0, 1, 2, 3], [0, 2, 1, 3], [0, 3, 1, 2]];

        matchTemplates.forEach((indices, idx) => {
          matches.push({
            id: uuidv4(),
            tournament_id: tournamentId,
            player_ids: indices.map(idx => podPlayers[idx].id),
            score1: 0,
            score2: 0,
            status: 'PENDING',
            round: 0,
            pod_id: podId,
            pod_name: podName,
            match_index: idx
          });
        });
      }

      if (matches.length > 0) {
        const { error: mError } = await supabaseAdmin.from('matches').insert(matches);
        if (mError) throw mError;
      }

      await supabaseAdmin.from('tournaments').update({ status: 'SEEDING' }).eq('id', tournamentId);
      res.json({ success: true });
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
  });

  app.post('/api/admin/submit-score', async (req, res) => {
    try {
      const { podId, matches, tournamentId } = req.body;
      console.log(`[Admin] Submitting scores for pod: ${podId} in tournament: ${tournamentId}`);

      // 1. Update matches
      for (const match of matches) {
        const { error } = await supabaseAdmin
          .from('matches')
          .update({
            score1: match.score1,
            score2: match.score2,
            status: 'LOCKED'
          })
          .eq('id', match.id);
        if (error) throw error;
      }

      // 2. Recalculate rankings for the tournament
      const { data: allMatches, error: mError } = await supabaseAdmin
        .from('matches')
        .select('*')
        .eq('tournament_id', tournamentId)
        .eq('status', 'LOCKED');
      if (mError) throw mError;

      const { data: players, error: pError } = await supabaseAdmin
        .from('players')
        .select('*')
        .eq('tournament_id', tournamentId);
      if (pError) throw pError;

      const playerStats = new Map<string, any>();
      players.forEach(p => {
        playerStats.set(p.id, { 
          points: 0, 
          pointDiff: 0, 
          pointsScored: 0, 
          podWins: 0,
          matchesPlayed: 0
        });
      });

      // Group matches by pod to calculate pod wins
      const podMatches = new Map<string, any[]>();
      allMatches.forEach(m => {
        if (!podMatches.has(m.pod_id)) podMatches.set(m.pod_id, []);
        podMatches.get(m.pod_id)!.push(m);
        
        // Update individual match stats
        m.player_ids.forEach((pid: string, idx: number) => {
          const stats = playerStats.get(pid);
          if (!stats) return;
          stats.matchesPlayed++;
          if (idx < 2) { // Team 1
            stats.pointsScored += m.score1;
            stats.pointDiff += (m.score1 - m.score2);
            if (m.score1 > m.score2) stats.points += 1;
          } else { // Team 2
            stats.pointsScored += m.score2;
            stats.pointDiff += (m.score2 - m.score1);
            if (m.score2 > m.score1) stats.points += 1;
          }
        });
      });

      // Calculate pod wins
      podMatches.forEach((matches, pid) => {
        const podPlayerPoints = new Map<string, number>();
        matches.forEach(m => {
          m.player_ids.forEach((pid: string, idx: number) => {
            const current = podPlayerPoints.get(pid) || 0;
            if (idx < 2) {
              if (m.score1 > m.score2) podPlayerPoints.set(pid, current + 1);
            } else {
              if (m.score2 > m.score1) podPlayerPoints.set(pid, current + 1);
            }
          });
        });

        // Find winner(s) of the pod
        let maxWins = -1;
        podPlayerPoints.forEach(wins => { if (wins > maxWins) maxWins = wins; });
        if (maxWins > 0) {
          podPlayerPoints.forEach((wins, pid) => {
            if (wins === maxWins) {
              const stats = playerStats.get(pid);
              if (stats) stats.podWins += 1;
            }
          });
        }
      });

      // Sort players to assign ranks
      const sortedPlayers = [...players].sort((a, b) => {
        const sA = playerStats.get(a.id);
        const sB = playerStats.get(b.id);
        if (sB.points !== sA.points) return sB.points - sA.points;
        if (sB.pointDiff !== sA.pointDiff) return sB.pointDiff - sA.pointDiff;
        return sB.pointsScored - sA.pointsScored;
      });

      // Update players in DB
      for (let i = 0; i < sortedPlayers.length; i++) {
        const p = sortedPlayers[i];
        const stats = playerStats.get(p.id);
        await supabaseAdmin
          .from('players')
          .update({
            points: stats.points,
            point_diff: stats.pointDiff,
            points_scored: stats.pointsScored,
            pod_wins: stats.podWins,
            rank: i + 1,
            last_rank: p.rank // Store previous rank for delta
          })
          .eq('id', p.id);
      }

      res.json({ success: true });
    } catch (err: any) {
      console.error('[Admin] Submit Score Error:', err);
      res.status(500).json({ error: err.message });
    }
  });

  app.post('/api/admin/advance-round', async (req, res) => {
    try {
      const { tournamentId } = req.body;
      console.log(`[Admin] Advancing round for tournament: ${tournamentId}`);

      const { data: currentTournament, error: tError } = await supabaseAdmin
        .from('tournaments')
        .select('*')
        .eq('id', tournamentId)
        .single();

      if (tError) {
        console.error('[Admin] Tournament fetch error:', tError);
        throw tError;
      }

      if (currentTournament.status === 'PLAYOFFS') {
        console.log('[Admin] Tournament is in PLAYOFFS status, checking for matches...');
        const { data: existingMatches } = await supabaseAdmin
          .from('playoff_matches')
          .select('id')
          .eq('tournament_id', tournamentId);
        
        if (!existingMatches || existingMatches.length === 0) {
          console.log('[Admin] No playoff matches found, generating from teams...');
          const { data: teams, error: teamsError } = await supabaseAdmin
            .from('playoff_teams')
            .select('*')
            .eq('tournament_id', tournamentId);
          
          if (teamsError) {
            console.error('[Admin] Playoff teams fetch error:', teamsError);
            throw teamsError;
          }

          console.log(`[Admin] Found ${teams?.length || 0} playoff teams`);
          
          if (teams && teams.length === 4) {
             const semiMatches = [
               {
                 id: uuidv4(),
                 tournament_id: tournamentId,
                 team1_id: teams[0].id,
                 team2_id: teams[3].id,
                 stage: 'SEMIS',
                 status: 'PENDING',
                 score1: 0,
                 score2: 0
               },
               {
                 id: uuidv4(),
                 tournament_id: tournamentId,
                 team1_id: teams[1].id,
                 team2_id: teams[2].id,
                 stage: 'SEMIS',
                 status: 'PENDING',
                 score1: 0,
                 score2: 0
               }
             ];
             
             const finalMatch = {
               id: uuidv4(),
               tournament_id: tournamentId,
               team1_id: null,
               team2_id: null,
               stage: 'FINALS',
               status: 'LOCKED',
               score1: 0,
               score2: 0
             };
             
             const { error: insertError } = await supabaseAdmin.from('playoff_matches').insert([...semiMatches, finalMatch]);
             if (insertError) {
               console.error('[Admin] Playoff matches insert error:', insertError);
               throw insertError;
             }
             console.log('[Admin] Playoff matches generated successfully');
             return res.json({ success: true, message: 'Playoff matches generated' });
          } else {
            console.warn(`[Admin] Cannot generate playoffs: ${teams?.length || 0} teams found, need 4`);
            return res.status(400).json({ error: `Draft not complete. Found ${teams?.length || 0} teams, need 4.` });
          }
        }
        console.log('[Admin] Playoff matches already exist');
        return res.json({ success: true, message: 'Matches already exist' });
      }

      const { data: players, error: pError } = await supabaseAdmin
        .from('players')
        .select('*')
        .eq('tournament_id', tournamentId)
        .order('rank', { ascending: true });
      if (pError) throw pError;

      if (currentTournament.current_round_index < 3) {
        const nextRound = currentTournament.current_round_index + 1;
        const nextMatches: any[] = [];

        for (let i = 0; i < players.length; i += 4) {
          const podPlayers = players.slice(i, i + 4);
          if (podPlayers.length < 4) break;

          const nextPodId = uuidv4();
          const podName = String.fromCharCode(65 + (i / 4));
          const matchTemplates = [[0, 1, 2, 3], [0, 2, 1, 3], [0, 3, 1, 2]];

          matchTemplates.forEach((indices, idx) => {
            nextMatches.push({
              id: uuidv4(),
              tournament_id: tournamentId,
              player_ids: indices.map(idx => podPlayers[idx].id),
              score1: 0,
              score2: 0,
              status: 'PENDING',
              round: nextRound,
              pod_id: nextPodId,
              pod_name: podName,
              match_index: idx
            });
          });
        }

        if (nextMatches.length > 0) {
          await supabaseAdmin.from('matches').insert(nextMatches);
        }

        await supabaseAdmin
          .from('tournaments')
          .update({ 
            current_round_index: nextRound,
            status: 'LADDER'
          })
          .eq('id', tournamentId);
      } else {
        await supabaseAdmin
          .from('tournaments')
          .update({ status: 'PLAYOFFS' })
          .eq('id', tournamentId);
      }

      res.json({ success: true });
    } catch (err: any) {
      console.error('[Admin] Advance Round Error:', err);
      res.status(500).json({ error: err.message });
    }
  });

  app.post('/api/admin/submit-playoff-score', async (req, res) => {
    try {
      const { matchId, score1, score2 } = req.body;
      const { data: match, error: mError } = await supabaseAdmin
        .from('playoff_matches')
        .update({ score1, score2, status: 'LOCKED' })
        .eq('id', matchId)
        .select()
        .single();
      if (mError) throw mError;

      if (match.stage === 'SEMIS') {
        const { data: semis } = await supabaseAdmin
          .from('playoff_matches')
          .select('*')
          .eq('tournament_id', match.tournament_id)
          .eq('stage', 'SEMIS');
        
        if (semis && semis.every(s => s.status === 'LOCKED')) {
          const winners = semis.map(s => s.score1 > s.score2 ? s.team1_id : s.team2_id);
          await supabaseAdmin
            .from('playoff_matches')
            .update({ 
              team1_id: winners[0], 
              team2_id: winners[1], 
              status: 'PENDING' 
            })
            .eq('tournament_id', match.tournament_id)
            .eq('stage', 'FINALS');
        }
      }

      if (match.stage === 'FINALS') {
        await supabaseAdmin.from('tournaments').update({ status: 'FINISHED' }).eq('id', match.tournament_id);
      }
      res.json({ success: true });
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
  });

  app.post('/api/admin/draft-partner', async (req, res) => {
    try {
      const { tournamentId, captainId, partnerId } = req.body;
      const { data: captain } = await supabaseAdmin.from('players').select('name').eq('id', captainId).single();
      const { data: partner } = await supabaseAdmin.from('players').select('name').eq('id', partnerId).single();
      
      const { error } = await supabaseAdmin.from('playoff_teams').insert([{
        tournament_id: tournamentId,
        captain_id: captainId,
        partner_id: partnerId,
        name: `${captain?.name} / ${partner?.name}`
      }]);
      if (error) throw error;
      res.json({ success: true });
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
  });

  app.post('/api/admin/reset-tournament', async (req, res) => {
    try {
      const { id } = req.body;
      await supabaseAdmin.from('matches').delete().eq('tournament_id', id);
      await supabaseAdmin.from('playoff_matches').delete().eq('tournament_id', id);
      await supabaseAdmin.from('playoff_teams').delete().eq('tournament_id', id);
      await supabaseAdmin.from('tournaments').update({ status: 'SETUP', current_round_index: 0 }).eq('id', id);
      await supabaseAdmin.from('players').update({ points: 0, point_diff: 0, points_scored: 0, pod_wins: 0, rank: 0 }).eq('tournament_id', id);
      res.json({ success: true });
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
  });

  app.post('/api/admin/finish-tournament', async (req, res) => {
    try {
      const { id } = req.body;
      const { error } = await supabaseAdmin
        .from('tournaments')
        .update({ status: 'FINISHED' })
        .eq('id', id);
      if (error) throw error;
      res.json({ success: true });
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
  });

  app.post('/api/admin/delete-tournament', async (req, res) => {
    try {
      const { id } = req.body;
      const { error } = await supabaseAdmin.from('tournaments').delete().eq('id', id);
      if (error) throw error;
      res.json({ success: true });
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
  });

  // Vite middleware for development
  if (process.env.NODE_ENV !== 'production') {
    const vite = await createViteServer({
      server: { middlewareMode: true },
      appType: 'spa',
    });
    app.use(vite.middlewares);
  } else {
    const distPath = path.join(process.cwd(), 'dist');
    app.use(express.static(distPath));
    app.get('*', (req, res) => {
      res.sendFile(path.join(distPath, 'index.html'));
    });
  }

  app.listen(PORT, '0.0.0.0', () => {
    console.log(`Server running on http://localhost:${PORT}`);
  });
}

startServer();
